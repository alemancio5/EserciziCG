...
/**************
 ...
 while it is at -500 for isometric, dimetric, trimetric and cabinet.
 The far plane is always at 500.
 For the parallel projections, the half-width of the screen in world coordinates is assumed to be 20.
...
***************/
	...
	// Isometric
    M = glm::mat4(1.0f / 20.0f,0,0,0,  0,-4.0f / 60.f,0,0, 0,0,1.0f/(-500-500.0f), 0,0,0,-500.0f/(-500-500.0f),1) *
        glm::mat4(1, 0, 0, 0, 0, glm::cos(glm::radians(-35.26f)), -glm::sin(glm::radians(-35.26f)), 0, 0, glm::sin(glm::radians(-35.26f)), glm::cos(glm::radians(-35.26f)), 0, 0, 0, 0, 1) *
        glm::mat4(glm::cos(glm::radians(45.0f)), 0, -glm::sin(glm::radians(45.0f)), 0, 0, 1, 0, 0, glm::sin(glm::radians(45.0f)), 0, glm::cos(glm::radians(45.0f)), 0, 0, 0, 0, 1);
	A->SetMatrix(1,  M);

	// Dimetric, with an angle of 20 degree
    M = glm::mat4(1.0f / 20.0f,0,0,0,  0,-4.0f / 60.f,0,0, 0,0,1.0f/(-500-500.0f), 0,0,0,-500.0f/(-500-500.0f),1) *
        glm::mat4(1, 0, 0, 0, 0, glm::cos(glm::radians(-20.0f)), -glm::sin(glm::radians(-20.0f)), 0, 0, glm::sin(glm::radians(-20.0f)), glm::cos(glm::radians(-20.0f)), 0, 0, 0, 0, 1) *
        glm::mat4(glm::cos(glm::radians(45.0f)), 0, -glm::sin(glm::radians(45.0f)), 0, 0, 1, 0, 0, glm::sin(glm::radians(45.0f)), 0, glm::cos(glm::radians(45.0f)), 0, 0, 0, 0, 1);
    A->SetMatrix(2 ,  M);
	
	// Trimetric, with an angle of alpha of 30 degree, and beta of 60 degrees
    M = glm::mat4(1.0f / 20.0f,0,0,0,  0,-4.0f / 60.f,0,0, 0,0,1.0f/(-500-500.0f), 0,0,0,-500.0f/(-500-500.0f),1) *
        glm::mat4(1, 0, 0, 0, 0, glm::cos(glm::radians(-30.0f)), -glm::sin(glm::radians(-30.0f)), 0, 0, glm::sin(glm::radians(-30.0f)), glm::cos(glm::radians(-30.0f)), 0, 0, 0, 0, 1) *
        glm::mat4(glm::cos(glm::radians(60.0f)), 0, -glm::sin(glm::radians(60.0f)), 0, 0, 1, 0, 0, glm::sin(glm::radians(60.0f)), 0, glm::cos(glm::radians(60.0f)), 0, 0, 0, 0, 1);
    A->SetMatrix(3 ,  M);

	// Create a Cabinet projection, with the z axis at an angle of 45 degrees
    M = glm::mat4(1.0f / 20.0f,0,0,0,  0,-4.0f / 60.f,0,0, 0,0,1.0f/(-500-500.0f), 0,0,0,-500.0f/(-500-500.0f),1) *
        glm::mat4(1, 0, 0, 0, 0, 1, 0, 0, -0.5f*glm::cos(glm::radians(45.0f)), -0.5f*glm::sin(glm::radians(45.0f)), 1, 0, 0, 0, 0, 1);
	A->SetMatrix(4,  M);
	
	// Create a perspective projection, with a Fov-y of  90 degrees
	M = glm::mat4(1/((4/3)*glm::tan(glm::radians(90.0f/2.0f))), 0, 0, 0, 0, -1/glm::tan(glm::radians(90.0f/2.0f)), 0, 0, 0, 0, 500/(0.1f - 500), -1, 0, 0, 0.1f*500/(0.1f - 500), 0);
    A->SetMatrix(5,  M);
	
	// Create a perspective projection, with a Fov-y of  30 degrees (zoom)
    M = glm::mat4(1/((4/3)*glm::tan(glm::radians(30.0f/2.0f))), 0, 0, 0, 0, -1/glm::tan(glm::radians(30.0f/2.0f)), 0, 0, 0, 0, 500/(0.1f - 500), -1, 0, 0, 0.1f*500/(0.1f - 500), 0);
	A->SetMatrix(6,  M);
	
	// Create a perspective projection, with a Fov-y of 120 degrees (wide)
    M = glm::mat4(1/((4/3)*glm::tan(glm::radians(120.0f/2.0f))), 0, 0, 0, 0, -1/glm::tan(glm::radians(120.0f/2.0f)), 0, 0, 0, 0, 500/(0.1f - 500), -1, 0, 0, 0.1f*500/(0.1f - 500), 0);
	A->SetMatrix(7,  M);
}

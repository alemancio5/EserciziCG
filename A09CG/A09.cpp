...
std::vector<SingleText> outText = {
	{2, {"Smooth Meshes showcase", "Press SPACE to see the Blocky Beast"}, 0, 0},
	{2, {"Smooth Blocky Beast", "Press SPACE to save the screenshots","",""}, 0, 0},
...
/*		std::vector<glm::vec3> vertices_pos = {
					   ...
					   { 10.0,0.0f, 10.0}};
		M[0].indices = {0, 1, 2,    1, 3, 2};*/
		std::vector<std::array<float,6>> vertices_pos[9];
...
//std::cout << cl[0] << "\n";
		...
		for(int j = 0; j < 9; j++) {
			...
			for(int i = 0; i < vertices_pos[j].size(); i++) {
				...
				V_vertex->pos.x  = vertices_pos[j][i][0];
				V_vertex->pos.y  = vertices_pos[j][i][1];
				V_vertex->pos.z  = vertices_pos[j][i][2];
				V_vertex->UV.x   = vertices_pos[j][i][0];
				V_vertex->UV.y   = vertices_pos[j][i][2];
				V_vertex->norm.x = vertices_pos[j][i][3];		
				V_vertex->norm.y = vertices_pos[j][i][4];		
				V_vertex->norm.z = vertices_pos[j][i][5];		

//printVec3("V",vertices_pos[i]);			
			}
			M[j].initMesh(this, &VD);

			MN[j].vertices = std::vector<unsigned char>(vertices_pos[j].size()*3*sizeof(Vertex), 0);
			MN[j].indices  = std::vector<uint32_t>(vertices_pos[j].size()*3*sizeof(uint32_t), 0);
			for(int i = 0; i < vertices_pos[j].size(); i++) {
				Vertex *V_vertex = (Vertex *)(&(MN[j].vertices[i*3 * mainStride]));
				V_vertex->pos.x  = vertices_pos[j][i][0]-0.02;
				V_vertex->pos.y  = vertices_pos[j][i][1];
				V_vertex->pos.z  = vertices_pos[j][i][2];
				V_vertex->UV.x   = vertices_pos[j][i][0];
				V_vertex->UV.y   = vertices_pos[j][i][2];
				...
				V_vertex = (Vertex *)(&(MN[j].vertices[(i*3+1) * mainStride]));
				V_vertex->pos.x  = vertices_pos[j][i][0]+0.02;
				V_vertex->pos.y  = vertices_pos[j][i][1];
				V_vertex->pos.z  = vertices_pos[j][i][2];
				V_vertex->UV.x   = vertices_pos[j][i][0];
				...
				V_vertex = (Vertex *)(&(MN[j].vertices[(i*3+2) * mainStride]));
				V_vertex->pos.x  = vertices_pos[j][i][0] + vertices_pos[j][i][3] * 0.2f;
				V_vertex->pos.y  = vertices_pos[j][i][1] + vertices_pos[j][i][4] * 0.2f;
				V_vertex->pos.z  = vertices_pos[j][i][2] + vertices_pos[j][i][5] * 0.2f;
				V_vertex->UV.x   = vertices_pos[j][i][0];
				V_vertex->UV.y   = vertices_pos[j][i][2];
				V_vertex->norm.x = vertices_pos[j][i][3];		
				V_vertex->norm.y = vertices_pos[j][i][4];		
				...
				MN[j].indices[i*3  ] = i*3;
				MN[j].indices[i*3+1] = i*3+1;
				MN[j].indices[i*3+2] = i*3+2;

//printVec3("V",vertices_pos[i]);			
			}
			MN[j].initMesh(this, &VD);
...
/*for(int i = 0; i < M[0].indices.size(); i++) {
	...
	void populateCommandBuffer(VkCommandBuffer commandBuffer, int currentImage) {
		...
		if((currScene == 1) || ((currScene == 2) && (subpass >= 8))) {
			for(int i = 0; i < 27; i++) {
				M[JM2M[i]].bind(commandBuffer);
				DS[i].bind(commandBuffer, WireFrame ? P : PW, 0, currentImage);
							
				vkCmdDrawIndexed(commandBuffer,
						static_cast<uint32_t>(M[JM2M[i]].indices.size()), 1, 0, 0, 0);
			}
		} else if((currScene == 0) || ((currScene == 2) && (subpass < 8))) {
			for(int i = 0; i < 9; i++) {
		...
		if(ShowVertNorm) {
			...
			if((currScene == 1) || ((currScene == 2) && (subpass >= 8))) {
				for(int i = 0; i < 27; i++) {
					MN[JM2M[i]].bind(commandBuffer);
					DS[i].bind(commandBuffer, PW, 0, currentImage);
								
					vkCmdDrawIndexed(commandBuffer,
							static_cast<uint32_t>(MN[JM2M[i]].indices.size()), 1, 0, 0, 0);
				}
			} else if((currScene == 0) || ((currScene == 2) && (subpass < 8))) {
				for(int i = 0; i < 9; i++) {
					MN[i].bind(commandBuffer);
					DS[i].bind(commandBuffer, PW, 0, currentImage);
								
					vkCmdDrawIndexed(commandBuffer,
							static_cast<uint32_t>(MN[i].indices.size()), 1, 0, 0, 0);
				}
			}
		}
	...
	void updateUniformBuffer(uint32_t currentImage) {
		...
		if(currScene == 2) {
			subpassTimer += deltaT;
			if(subpassTimer > 5.0f) {
				subpassTimer = 0.0f;
				subpass++;
				std::cout << "Scene : " << currScene << " subpass: " << subpass << "\n";
				char buf[20];
				sprintf(buf, "A09_%d.png", subpass);
